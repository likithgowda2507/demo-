        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages):
                ...
                for table in tables:
                    ...
                    if not has_raci_header and current_headers and rows:
                        # Skip page metadata tables (headers and footers) without breaking continuation
                        if "document title:" in header_text or "document classification:" in header_text:
                            continue

                        # Check if this table is actually a different type of table (like SIPOC or revision history)
                        sipoc_keywords = ["supplier", "input", "process", "output", "customer"]
                        is_sipoc = sum(1 for k in sipoc_keywords if k in header_text) >= 3 or "sipoc" in header_text
                        is_history = "revision history" in header_text or "amendment record" in header_text
                        
                        looks_like_new_table = (
                            is_sipoc or is_history or 
                            any(str(h).strip().lower() in [
                                "description", "action", "step", "activity", 
                                "task", "sub-task", "supplier", "customer", 
                                "pitfall", "impact", "solution"
                            ] for h in raw_headers[:5])
                        )
                        
                        if looks_like_new_table:
                            if current_headers and current_rows:
                                table_md = self._format_markdown_table(current_headers, current_rows)
                                page_label = ", ".join(str(p) for p in current_pages)
                                final_tables.append(f"--- Page {page_label} ---\n{table_md}")
                                result = "\n\n".join(final_tables)
                                return self._clean_table_output(result)
                            current_headers = None # End RACI collection
                            current_rows = []
                            continue

                        # Include first row as data (table[0] is data, not a header on continuation pages)
                        first_row_data = []
                        for cell in table[0]:
                            if cell:
                                first_row_data.append(str(cell).replace("\n", " ").strip())
                            else:
                                first_row_data.append("")
                        all_rows = [first_row_data] + rows
                    ...
                    # Collapse merged columns from PDF merged cells
                    # e.g. 15 cols ['', 'Process', '', '', 'Responsible', '', '', ...] → 5 cols
                    named_positions = [i for i, h in enumerate(raw_headers) if h and h.strip()]
                    if len(named_positions) >= 3:
                        spacings = [named_positions[i+1] - named_positions[i] for i in range(len(named_positions)-1)]
                        if len(set(spacings)) == 1 and spacings[0] > 1:
                            group_size = spacings[0]
                            offset_in_group = named_positions[0]  # position of first header within its group
                            collapsed_headers = [raw_headers[p] for p in named_positions]
                            collapsed_rows = []
                            for row in rows:
                                crow = []
                                for p in named_positions:
                                    gs = p - offset_in_group  # group start
                                    ge = gs + group_size  # group end (exclusive)
                                    val = ""
                                    for ci in range(gs, min(ge, len(row))):
                                        if ci < len(row) and row[ci] and str(row[ci]).strip():
                                            val = str(row[ci]).strip()
                                            break
                                    crow.append(val)
                                collapsed_rows.append(crow)
                            raw_headers = collapsed_headers
                            rows = collapsed_rows
